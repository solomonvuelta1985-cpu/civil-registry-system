...
requireAuth();
requireCSRFToken();
if (!hasPermission('birth_create')) { json_response(false, 'Permission denied.', null, 403); }
...
try {
    ...
    // date_of_marriage_others column is always nulled on new saves; partial state
    ...
    if ($date_of_marriage_format === 'full') {
        // Full date is optional — leave null if not provided.
    ...
    try {
        ...
        $sql = "INSERT INTO certificate_of_live_birth (
                    ...
                    date_of_marriage_format,
                    date_of_marriage_partial_month,
                    date_of_marriage_partial_year,
                    date_of_marriage_partial_day,
                    place_of_marriage,
                    pdf_filename,
                    pdf_filepath,
                    pdf_hash,
        ...
        // Run duplicate detection (non-blocking — failures don't affect save)
        $potential_duplicates = [];
        try {
            $potential_duplicates = find_potential_duplicates($pdo, $inserted_id, 'birth');
